...
extension Tensor.Value where Element: Copyable {
    ...
    public func map<NewElement: Copyable, E: Swift.Error>(
        ...
        // Element: Copyable). The for-each loop preserves the typed throw shape
        // because the for-in body propagates `throws(E)` natively.
        for element in self._storage {
            storage.append(try transform(element))
        }
